...
- Committing staged credentials MUST increment `session_gen` to invalidate old sessions.
- Factory reset MUST revert to `admin`/`admin123` and increment `session_gen`.
...
5. Add `bool nvs_store_commit_staging(void);`. Reads `stg_type`. If 1: call `nvs_store_save_wifi_profile(stg_sta_ssid, stg_sta_pass)` to officially append it to the profile list. If 2: copy `stg_admin_user`/`pass` to `admin_user`/`pass` and increment `session_gen` (to invalidate old sessions). Then set `stg_type=0`.
...
- `nvs_store_commit_staging` correctly increments `session_gen` when committing credentials.
...
4. Increments the NVS key `session_gen` (invalidates all sessions).
5. Clears any pending staging by setting `stg_type=0`.
6. Commits changes.
</action>
<acceptance_criteria>
- `nvs_store_factory_reset_credentials` correctly writes default credentials and bumps `session_gen`.
</acceptance_criteria>
</task>
